...
export type AnswerEditTarget = {
  /** interview_messages 의 user 메시지 id (= K 응답의 answerId) */
  messageId: string;
  /** 화면 표시용 Q 번호 (1-based) */
  questionNo: number;
  question: string;
...
export function AnswerEditModal({
  ...
  const activeId = open ? (target?.messageId ?? null) : null;
  ...
  const mutation = useMutation({
    mutationFn: (content: string) => {
      ...
      return interviewApi.patchMessage(sessionId, target.messageId, content);
    },
    onSuccess: async () => {
    ...
    mutation.mutate(trimmed, {
      onSuccess: () => {
        ...
        if (then) {
          ...
          toast.success('답변을 수정했어요', { msg: '개요의 인터뷰 기록이 갱신됐어요.' });
        }
      },
      onError: () => {
        toast.error('답변 수정 실패', { msg: '잠시 후 다시 시도해주세요.' });
      },
    });
  };
      ...
      <div className="px-[22px] pb-2 pt-[22px]">
        <div className="flex items-start gap-[14px]">
          ...
          <div className="min-w-0 flex-1">
            ...
            >
              답변 수정 {target ? `· Q${target.questionNo}` : ''}
            </div>
            <h2
              id={titleId}
